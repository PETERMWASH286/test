...
Future<void> _signup() async {
  try {
    ...
    // Perform the POST request with the token in the headers
    final response = await http.post(
      Uri.parse('https://expertstrials.xyz/Garifix_app/switch_signup'),
...
// Remove the existing JWT token if it exists
if (prefs.containsKey('jwt_token')) {
  await prefs.remove('jwt_token');
  print('Existing token cleared.');
}

// Remove the existing user role if it exists
if (prefs.containsKey('user_role')) {
  await prefs.remove('user_role');
  print('Existing user role cleared.');
}

// Store the new JWT token
await prefs.setString('jwt_token', token);
print('Token stored: $token'); // Log the stored token for debugging
